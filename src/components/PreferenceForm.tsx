...
export default function PreferenceForm({ keyword, initial, onSubmit, loading }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-orange-100 bg-white p-6 shadow-sm">
      <div>
        <h2 className="type-section-title">个性化偏好问卷</h2>
        <p className="type-subtitle">关键词: <strong>{keyword}</strong></p>
      </div>
      ...
      <fieldset>
        <legend className="type-label mb-2">增值服务需求（数码类）</legend>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className="type-label mb-1 block">商品类目</label>
          <select name="category" defaultValue={initial?.category || "digital"} className="type-input w-full rounded-lg border border-gray-200 p-3">
