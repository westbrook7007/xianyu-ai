-- 一次性创建全部业务表（在 Supabase SQL Editor 中 Run）
-- 若已存在 crawl_queue，可安全重复执行（IF NOT EXISTS）
...
CREATE TABLE IF NOT EXISTS product_data (
  ...
  seller_type INT DEFAULT 1,
  life_level INT DEFAULT 3,
...
DROP POLICY IF EXISTS "Public read products" ON product_data;
CREATE POLICY "Public read products" ON product_data FOR SELECT USING (true);
DROP POLICY IF EXISTS "Public read trends" ON price_trend;
CREATE POLICY "Public read trends" ON price_trend FOR SELECT USING (true);
DROP POLICY IF EXISTS "Service insert products" ON product_data;
CREATE POLICY "Service insert products" ON product_data FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Service insert trends" ON price_trend;
